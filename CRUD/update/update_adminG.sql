...
delimiter //
create function actualizar_adminG(_id int, _correo varchar(50), _telefono varchar(10), _ciudad varchar(50), _contrasena varchar(10))
    returns int
begin
    ...
    if _telefono is not null then
        if _contrasena is not null then
            update administrador_general set correo = _correo, telefono = _telefono, cod_postal = _cod_postal, contrasena = _contrasena where id = _id;
            return 0;
        else 
            return 1;
        end if;
